    ...
    def evaluate(
        ...
        for b_idx, batch in enumerate(eval_loader):
            ...
            with torch.no_grad():
                ...
                total_time = total_time + batch['val_time']
                # print('total val time: {} s'.format(batch['val_time']))
        ...
        # // JRDB 2019: train=21744; val=6203; test=27661;
        # // JRDB 2022: train=21704; val=6189; test=27661;
        print('total val time: {} s'.format(total_time))
        print('JRDB 2019 avg 6203 val FPS: {} s'.format(6203/total_time))
        print('JRDB 2019 avg 6189 val FPS: {} s'.format(6189/total_time))
        # prevent out-of-memory during evaluation
        torch.cuda.empty_cache()
            ...
            if not self.__sigterm:
                ...
                if do_ckpt:
                    self._logger.save_ckpt(
                        ...
                        # 2023-03-22 Jinzheng Guang
                        'ckpt_e{:02d}.pth'.format(self._epoch),
                        model,
                        self._optim,
                        self._epoch,
                        self._step,
